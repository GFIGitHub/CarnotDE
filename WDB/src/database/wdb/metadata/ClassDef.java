...
public class ClassDef extends Query implements Serializable {
	...
	public void addIndex(IndexDef index, Adapter adapter) throws Exception
	{
		indexes.add(index);
		this.commit(adapter);
	}
	...
	public boolean isSubclassOf(String _parent, Adapter adapter) throws Exception
	{
		return false;
	}
	...
	public WDBObject newInstance(WDBObject parent, Adapter adapter) throws Exception
	{
		if(parent != null)
		{
	...
	{
		adapter.putClass(this);
	}
	public WDBObject[] search(SimpleNode expression, Adapter adapter) throws Exception
	{
        ...
        if (hasWhereClause) {
		    indexFilteredArray = expression.filterObjectsWithIndexes(adapter, this.indexes).getFilteredResults(adapter, this.indexes);
        }
        else {
		...
		if(indexFilteredArray == null)
		{
			...
			{
				object = this.getInstance(i, adapter);
				if( !hasWhereClause 
                    || (hasWhereClause && expression.eval(adapter, object)) )
				{
		...
		else
		{
			...
			{
				if(!hasWhereClause
                    || (hasWhereClause && expression.eval(adapter, indexFilteredArray[i])))
				{
	...
	public void padAttribute(PrintNode row, AttributePath attributePath, Adapter adapter) throws Exception
	{
		...
		{	
			...
			if(currentAttribute.getClass() == EVA.class && attributePath.levelsOfIndirection() > 0)
			{
				...
				if(evaName.equals(currentAttribute.name))
				{
					...
					targetClass.padAttribute(row, attributePath, adapter);
		...
		if(!(attributePath.attribute.equals("*") && attributePath.levelsOfIndirection() <= 0))
		{
			...
			if(this.getClass() == SubclassDef.class)
			{
				...
				{
					ClassDef parentClass = adapter.getClass(((SubclassDef)this).getSuperClass(i));
					parentClass.padAttribute(row, attributePath, adapter);
				}
			}
			else
			{
