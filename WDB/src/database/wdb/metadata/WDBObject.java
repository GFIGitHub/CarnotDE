...
public class WDBObject implements Serializable {
	...
	public WDBObject getBaseObject(Adapter adapter) throws Exception
	{
		ClassDef myClass = this.getClassDef(adapter);
		...
		if(myClass.getClass() == SubclassDef.class)
		{
			...
			while(e.hasMoreElements())
			{
				...
				WDBObject parent = adapter.getObject(parentClass, parentUid);
				WDBObject grandparent = parent.getBaseObject(adapter);
				if(grandparent != null)
				{
					return grandparent;
				}
	...
	public WDBObject getParentObject(String superClassName, Adapter adapter) throws Exception
	{	
		ClassDef myClass = this.getClassDef(adapter);
		//See if its parent of this class
		if(myClass.getClass() == SubclassDef.class)
		{
			...
			{
				return adapter.getObject(superClassName, ((Integer)parents.get(superClassName)));
			}
			...
			while(e.hasMoreElements())
			{
				...
				Integer parentUid = (Integer)parents.get(parentClass);
				WDBObject parent = adapter.getObject(parentClass, parentUid);
				WDBObject grandparent = parent.getParentObject(superClassName, adapter);
				if(grandparent != null)
				{
					return grandparent;
				}
	...
	public WDBObject getChildObject(String subClassName, Adapter adapter) throws Exception
	{
		//See if its one of my immediate subclasses.
		if(children.containsKey(subClassName))
		{
			return adapter.getObject(subClassName, ((Integer)children.get(subClassName)));
		}
		else
		{
			return null;
	...
	{
		adapter.putObject(this);
	}
	public ClassDef getClassDef(Adapter adapter) throws Exception
	{
		return adapter.getClass(this.classDefName);
	}
	...
	public Object getDvaValue(String dvaName, Adapter adapter) throws Exception
	{
		Object value = null;
		...
		else if(myClass.getClass() == SubclassDef.class)
		{
			...
			while(true)
			{
				try
				{
					...
					value = parent.getDvaValue(dvaName, adapter);
	...
	public void setDvaValue(String dvaName, Object value, Adapter adapter) throws Exception
	{	
		//See if its immediate in this class
		ClassDef myClass = this.getClassDef(adapter);
    Attribute myAttribute = myClass.getAttribute(dvaName);
		...
		else if(myClass.getClass() == SubclassDef.class)
		{
			...
			while(true)
			{
				try
				{
					...
					parent.setDvaValue(dvaName, value, adapter);
	...
	public void addEvaObjects(String evaName, String targetClass, SimpleNode expression, Adapter adapter) throws Exception
	{
		ClassDef myClass = this.getClassDef(adapter);
		Attribute myAttribute = myClass.getAttribute(evaName);
		if(myAttribute != null && myAttribute.getClass() == EVA.class)
		{
			ClassDef targetClassDef = adapter.getClass(targetClass);
			if(targetClassDef.name.equals(((EVA)myAttribute).baseClassName) || 
					targetClassDef.isSubclassOf((((EVA)myAttribute).baseClassName), adapter))
			{
				ClassDef baseClassDef = adapter.getClass(((EVA)myAttribute).baseClassName);
				WDBObject baseObject;
				WDBObject[] matchingObjs = targetClassDef.search(expression, adapter);
				for(int i = 0; i < matchingObjs.length; i++)
				{
					this.addEvaObject((EVA)myAttribute, matchingObjs[i], adapter);
					...
					{
						baseObject = matchingObjs[i].getParentObject((((EVA)myAttribute).baseClassName), adapter);
					}
					...
					{
						baseObject.addEvaObject((EVA)inverseAttribute, this, adapter);
					}
					else
					{
		...
		else if(myClass.getClass() == SubclassDef.class)
		{
			...
			while(true)
			{
				try
				{
					...
					parent.addEvaObjects(evaName, targetClass, expression, adapter);
					//If we reached here, we set our value. Get out of here
					break;
				}
				catch(NoSuchFieldException nsfe)
	...
	public void removeEvaObjects(String evaName, String targetClass, SimpleNode expression, Adapter adapter) throws Exception
	{
		ClassDef targetClassDef = adapter.getClass(targetClass);
		WDBObject[] matchingObjs = targetClassDef.search(expression, adapter);
		removeEvaObjects(evaName, targetClass, matchingObjs, adapter);
	}
	
	public void removeEvaObjects(String evaName, String targetClass, WDBObject[] targetObjects, Adapter adapter) throws Exception
	{
			...
			if(myAttribute != null && myAttribute.getClass() == EVA.class)
			{
						...
						targetClassDef.isSubclassOf((((EVA)myAttribute).baseClassName), adapter))
				{
					...
					for(int i = 0; i < targetObjects.length; i++)
					{
						...
						if(this.removeEvaObject((EVA)myAttribute, targetObjects[i], adapter))
						{
							//Get the object that belongs to the base class of the EVA
							if((((EVA)myAttribute).baseClassName).equals(targetClassDef.name))
							{
							...
							{
								baseObject = targetObjects[i].getParentObject((((EVA)myAttribute).baseClassName), adapter);
							}
							//Get the inverse EVA, if there is one
							Attribute inverseAttribute = baseClassDef.getAttribute(((EVA)myAttribute).inverseEVA);
							//Put myself as a reference in the target's EVA
							if(inverseAttribute != null && inverseAttribute.getClass() == EVA.class)
							{
								baseObject.removeEvaObject((EVA)inverseAttribute, this, adapter);
							}
							else
							{
								throw new Exception("Implication of target class inverse EVAs is not implemented");
			...
			else if(myClass.getClass() == SubclassDef.class)
			{
				...
				while(true)
				{
					try
					{
						...
						parent.removeEvaObjects(evaName, targetClass, targetObjects, adapter);
						//If we reached here, we set our value. Get out of here
						break;
					}
					catch(NoSuchFieldException nsfe)
	...
	private void addEvaObject(EVA targetEva, WDBObject targetEvaObject, Adapter adapter) throws Exception
	{
	...
	private Boolean removeEvaObject(EVA targetEva, WDBObject targetEvaObject, Adapter adapter) throws Exception
	{
		if(targetEva.cardinality.equals(EVA.MULTIVALUED))
		{
...
/*
	private void addEvaInverse(EVA ownerEva, ClassDef ownerEvaClass, WDBObject ownerEvaObject, Adapter adapter) throws Exception
	{
	...
	*/
	public WDBObject[] getEvaObjects(String evaName, Adapter adapter) throws Exception
	{
		ClassDef myClass = this.getClassDef(adapter);
		Attribute myAttribute = myClass.getAttribute(evaName);
			...
			if(((EVA)myAttribute).cardinality.equals(EVA.MULTIVALUED))
			{
				...
				if(targetObjectList != null)
				{
					for(int i = 0; i < targetObjectList.size(); i++)
					{
						...
						WDBObject targetObject = adapter.getObject(targetClass, targetUid);
		...
		else if(myClass.getClass() == SubclassDef.class)
		{
			...
			while(true)
			{
				try
				{
					...
					return parent.getEvaObjects(evaName, adapter);
			...
			if(e.getClass() == NoSuchFieldException.class && classDef.getClass() == SubclassDef.class)
			{
				...
				try
				{
					...
					while(target_class == null && i < subclassDef.numberOfSuperClasses())
					{
						...
						{
							parent = adapter.GetWDBObject((Integer)parents.get(subclassDef.getSuperClass(i)), subclassDef.getSuperClass(i));
							target_class = parent.GetEVATarget_class(name);
						}
	...
	public boolean SetEVARef_to(String name, String targetClass, SimpleNode exp)
	{
		...
		if(attribute != null && attribute.getClass() == EVA.class)
		{
			...
			try
			{
				...
				ClassDef tc = adapter.GetClassDef(targetClass);
				ClassDef cd = adapter.GetClassDef(eva.baseClassName);
				...
				if(adapter.CheckSubclassMembership(tc, cd))
				{
					...
					{
						partner = adapter.GetWDBObject(tc.getInstance(i), tc.name);
						if(exp.eval(adapter, partner))
						{
	...
	*/
	...
	public ArrayList<Object> getAttributeValue(AttributePath attributePath, Adapter adapter) throws Exception
	{
		ArrayList<Object> values = new ArrayList<Object>();
		
		if(attributePath.levelsOfIndirection() <= 0)
		{
			values.add(this.getDvaValue(attributePath.attribute, adapter));
			return values;
		}
		else
		{
			String evaName = attributePath.getIndirection(attributePath.levelsOfIndirection() - 1);
			WDBObject[] objects = this.getEvaObjects(evaName, adapter);
			...
			if(objects != null && objects.length > 0)
			{
				...
				{
					values.addAll(objects[i].getAttributeValue(attributePath, adapter));
				}
	...
	public void PrintAttribute(PrintNode row, AttributePath attributePath, Adapter adapter) throws Exception
	{	
		ClassDef myClass = this.getClassDef(adapter);
		if(attributePath.levelsOfIndirection() <= 0)
		{
			if(attributePath.attribute.equals("*"))
			{
				...
				if(myClass.getClass() == SubclassDef.class)
				{
					for(int i = 0; i < ((SubclassDef)myClass).numberOfSuperClasses(); i++)
					{
						...
						WDBObject parent = adapter.getObject(parentClass, parentUid);
						parent.PrintAttribute(row, attributePath, adapter);
					}
				}
			}
			else
			{
				...
				Object dvaValue = getDvaValue(attributePath.attribute, adapter);
				PrintCell cell = new PrintCell();
				cell.setOutput(String.format("%s", dvaValue));
				row.addCell(cell);
			}
		...
		else
		{
			...
			if(objects != null && objects.length > 0)
			{
				...
				for(int i = 0; i < objects.length; i++)
				{
					objects[i].PrintAttribute(branch.get(i), attributePath, adapter);
				}
				attributePath.addIndirection(evaName);
			}
			else
			{
				if(branch == null)
				{
					...
					myClass.padAttribute(branch.get(0), attributePath, adapter);
				}
			}
